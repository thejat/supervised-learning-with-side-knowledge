...
function [betaCVX, bestModelCoeff, cvMatrix] = ...
    select_model_using_cv(strModelName, coeffRange, nFolds, nRepeats, sampleTrainX, sampleTrainY, knowledge)
    ...
    for i=1:length(coeffRange) %For each model
        for j=1:nFolds %For each fold
            ...
            elseif(strcmp(strModelName,'Ridge')==1)
                betaCVX = ridge_regression(XEverythingElse,YEverythingElse,coeffRange(i),knowledge);
...
elseif(strcmp(strModelName,'Ridge')==1)
    betaCVX = ridge_regression(sampleTrainX,sampleTrainY,bestModelCoeff,knowledge);
elseif(strcmp(strModelName,'SVR')==1)
